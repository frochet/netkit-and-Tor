...
if [ ! 0 ]; then
   ...
   if [ "/usr/local/share/netkitcore" != "$PWD" -a "/usr/local/share/netkitcore" != "${PWD}/" ]; then
      echo "failed!"
      echo
      echo "*** Error: the environment variable NETKIT_HOME currently points at the"
      echo "wrong directory:"
      echo
      echo "(current value) /usr/local/share/netkitcore"
      echo "(should be)     $PWD"
      echo
...
NETKIT_BIN_DIR=/usr/local/share/netkitcore/bin
